...
public class PlaylistsTracksController : ControllerBase
{
    ...
    {
        if (_context.PlaylistsTracks == null) return NotFound();
        var playlistsTracks = await _context.PlaylistsTracks.FindAsync(id);

        if (playlistsTracks == null) return NotFound();
    ...
    {
        if (id != playlistsTracks.PlaylistId) return BadRequest();
    ...
    {
        if (_context.PlaylistsTracks == null) return Problem("Entity set 'MusicAPIContext.PlaylistsTracks'  is null.");
